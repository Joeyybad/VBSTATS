...
const schema = yup.object({
  clubName: yup
    .string()
    .min(4, "Le nom du club doit être supérieur à 4")
    .required("Le nom du club est requis"),
  description: yup
    .string()
    .min(10, "La description doit avoir 10 caractères")
    .required("La description du club est requise"),
});
...
onMounted(async () => {
  ...
  if (clubStore.club) {
    ...
    if (clubStore.club.clubImg) {
      // On récupère l'URL de base depuis la config
      const apiUrl = api.defaults.baseURL;
      // On enlève le "/api" à la fin pour avoir la racine
      const serverUrl = apiUrl.replace(/\/api$/, "");

      previewUrl.value = `${serverUrl}/uploads/logos/${clubStore.club.clubImg}`;
    }
  }
  if (location.value) {
...
const onSubmit = handleSubmit(async (values) => {
  try {
    ...
    await await api.put("/club/update", formData, {
      headers: {
